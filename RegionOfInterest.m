classdef (CaseInsensitiveProperties = true) RegionOfInterest < hgsetget
	...
	methods % CONSTRUCTOR & SETUP
		...
		function varargout = updateProperties(obj)
			if any(cellfun(@isempty,{obj.PixelIdxList})) % doesn't change anything
				obj = removeEmpty(obj);
			end			
			updateCID(obj)			
			...
			set(obj, 'FrameSize', obj(end).FrameSize); % set frame size
			...
			% WEIGHTED CENTROID % don't use these
			...
			% FIRST FRAME and LAST FRAME % don't use these either
