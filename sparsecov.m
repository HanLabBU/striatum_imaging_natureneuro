function c = sparsecov(x,varargin) % modified from matlab
...
%
%   C = cov(..., MISSING) specifies how NaN (Not-A-Number) values are 
...
meanvals = sum(x,1)/m;
% xc = sparse(bsxfun(@minus,x,sum(x,1)/m));  % Remove mean
c = zeros(2);
step = 1000000;
for j=0:step:size(x,1)
    startind = j+1;
    indend = min(j+step,size(x,1));
    c = c + ((bsxfun(@minus,x(startind:indend,:),meanvals))' * ...
        (bsxfun(@minus,x(startind:indend,:),meanvals))) / denom;
    fprintf('%0.2d\n',j/size(x,1));
end    
