% some code borrowed/modified from https://gist.github.com/cholland29/3107790
function [goodIndices, badIndices] = markBadTraces(taxis, traces, nhat)
    goodIndices = [];
    badIndices = [];
    ...
    function OnClickAxes(hax, evt)
        ...
        allLines = findall(hax,'type','surface');
...
%     maxVal = max(traces(:));
    maxVal = 1;
    [H, surfaceCell] = plotActiveRegionsSimple(taxis, (traces/10), nhat');
    title([num2str(size(traces,1)) ' traces']);
    set(H.Children(1), 'ButtonDownFcn',@OnClickAxes);
    for lim=0:3:(length(surfaceCell))
        ylim([lim-.5 lim+2.5]);
        pause
    end
    badIndices = unique(badIndices);
    goodIndices = setdiff(1:size(traces,1), badIndices);
end
